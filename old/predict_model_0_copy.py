...
# python predict_model_0_copy.py f:/amicorpus_1 --model 10
#############
...
def get_silence(name, sec):
    ...
    seconds_all = [i for i in range(0, length[name])]

    seconds = [sec for sec in seconds_all if sec in seconds_sp] # not in - участки без речи, in участки с речью
...
if __name__ == '__main__':
    ...
    # compute audio
        ...
        length[name_f[k]] = (len(audio)//rate)//SEC
    ...
    # downmix to mono
        ...
        # Отметить, где нет речи
        seconds = get_silence(name_f[k], SEC)

        #length = (len(audio)//rate)//SEC
        #seconds_all = [i for i in range(0, length + 1)]

        #seconds = [sec for sec in seconds_all if sec not in seconds_sp]
