...
# python predict_model_0.py f:/amicorpus_1 --model 11
#############
...
def get_silence(name, sec):
    ...
    file_rttm = 'outputs\\logunov\\speech_regions_predicted_vad_markup_bad_repaired_0.5\\' + name + '.segments.rttm'
...
if __name__ == '__main__':
    ...
    path_to = 'AMI_amp4mean_without0_old_rttm_array1-01'  # withoun 0 - использование разметки csv с речевой активностью, old - секунды целочисленные, rttm - VAD Леши
    ...
    for k in range(1, 168): # 169 range(70, 80) len(path_f)
    ...
    # downmix to mono
        ...
        # добавить проверку на длительность файла?

        estimate = []
        for i in range(len(seconds)):
            sec_begin = int(seconds[i]*rate)
            estimate.append(count(audio[sec_begin:sec_begin+SEC*rate], model, scaler, SEC, AMP)) # -+100 (отступ назад и вперед)
            #print(seconds[i], estimate[-1])
    ...
    for name in name_f: # [:10] [70:80]
        ...
        true_labels = get_labels_AMI(name, SEC, noSilence = True)
